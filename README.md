...
# 謝辞
制作にあたり、以下を参考にさせて頂きました。ありがとうございました。
Amazonで一年間に使った金額と、注文履歴のTSVを出力するブックマークレット【2016年版】

# 開発の経緯

* 昨年（2016年)分の確定申告にあたって、一年間でAmazonで購入した書籍、文具、消耗品などが
注文単位で170件ぐらいあった。
* これらの注文を、購入品の明細単位で、経費管理システムのDB(MySQL)に投入したかった。
* 購入品単位で投入したいのは、経費に計上するものに勘定科目を付番したかったからです。
(たとえば、一回の注文の中に、書籍とプリンターのトナーがあった場合、
書籍は新聞図書費、トナーは消耗品費としたい。)
* が、Amazonの注文履歴や注文の詳細ページを目視して、WEB画面に投入するのを手作業ではやりたくない。
* そこで、プログラムから自動的に注文履歴のページをたどり、注文情報（品目、金額など）を
* 取得したのち、ローカルにMySQLに投入しやすい形式で、ファイルに保存するようなものを作りたいと考えました。


# 実現方法

* Selenium Webdriver を使ってFireFoxで注文履歴ページを巡回さて欲しいデータを取得
* 開発言語は Ruby 2.2
...
&nbsp;&nbsp;当プログラムは指定されたユーザーアカウントでAmazonにログインした状態で動作します。
もし、「悪意あるプログラムが動いて、誤発注などがあったら？」というご不安のある方は 使用しないでください。
&nbsp;&nbsp;また、このプログラムの実行によって、何らかの誤動作があり、利用者の不利益となるような状況が
発生しても開発者は責任を負いかねます。以上をご了解のもとでご使用をお願い致します。

# 前提

* [Selenium Webdriver](https://rubygems.org/gems/selenium-webdriver/) が必要です。なければ
...
(/Users/ykt68/work は、適宜、お手元の作業ディレクトに読み替えてください。)
...
* メインスクリプト、amachu_main.rb　は、以下のオプションを取ります。
    ...
    * -u <i><Amazonアカウント></i>　：必須
    
    * -p <i><パスワード></i> :必須
    
    * -y <i><西暦年></i> :省略可。省略した場合は、現在日時の西暦となります。<br />指定できる最小値は2000（=日本アマゾンのサービス開始の年）
    
    * -o <i><出力ディレクトリ></i> ：省略可。省略した場合は、```./orders``` というディレクトリが作成され、そこに出力されます。

# 現時点のバージョンの機能制限

* 制作者が自分のAmazonアカウントで実行した範囲では、書籍、日用雑貨、およびAmazonビデオしか購入品があり
<br>ませんでしたので、その範囲で対応できるプログラムになっています。<br />
　もしこれらの範疇ではない物品が含まれていた場合、注文データを取得できない場合があるかもしれません。
...
* 制作者の開発および実行環境は以下です。
  ...
  WindowsやLINUXなどの他のOSや、他のブラウザ、もしくはFireFoxの他のバージョンでは検証していません。
  
  
# 改修のポイント

お使いになる場合に、利用者の便宜に沿うように改修するために、ソースのどのあたりに<br />
手を入れたらいいかを挙げます。
